...
## Update 2026-07-19 (later): serialized tab loading fixes the rate-limiting, and every return row is now streamed

Two follow-up changes, both confirmed on a real run:

- **Rate-limiting was real** (2 of 5 tabs failed with a `page.goto` timeout in one run, with all 5 tabs' initial loads starting near-simultaneously, only 250ms apart). Fix: the expensive part — a tab's *initial* page load (`goto` + cookie-dismiss + wait for fares) — is now run **sequentially**, one tab fully loaded before the next tab's load even starts. Once a tab has loaded, its remaining work (commit to its outbound row, wait for the return page, read every return row) is a cheap in-place SPA update, not a fresh navigation, and runs **concurrently** across tabs — it doesn't have to wait for other tabs to finish loading. On a re-run with this change, all 5 tabs succeeded (vs. 2 failures before), total time ~40s for 5 outbound options.
- **Every return row is now streamed, not just the top 3** — reading a row is just `textContent()` + regex, no extra click/wait, so it's free once the return page has already loaded. One search now yields up to 50 cards (5 outbound x up to 10 return rows each) instead of 15.
